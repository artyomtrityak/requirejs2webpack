    ...
    componentDidMount: function () {
      ...
      'onStart onEnd onAdd onSort onUpdate onRemove onFilter'.split(' ').forEach(function (/** string */name) {
        copyOptions[name] = function (evt) {
          if (name === 'onStart') {
            // On this event all popovers should be disposed
            Chaplin.mediator.publish('ec:list:sort:start');
      ...
      var sortableRef = this.refs[options.ref] ? this.refs[options.ref] : ReactDOM.findDOMNode(this);
      this._sortableInstance = Sortable.create(sortableRef, copyOptions);
    },
