  ...
  var factory = function($) {
    ...
    var Toggles = root.Toggles = function(el, opts) {
      ...
      opts = self.opts = $.extend({
        ...
        'event': 'toggle',
        // callback for On/true toggle
        'onOn': null,
        // callback for Off/false toggle
        'onOff': null
      }, opts || {}, dataOpts);
    ...
    Toggles.prototype.toggle = function(state) {
      ...
      // set callback function depending on the state
      if (active && typeof self.opts.onOn === 'function')
        callback = self.opts.onOn;
      else if (!active && typeof self.opts.onOff === 'function')
        callback = self.opts.onOff;

      // move the toggle!
      self.els.inner.stop().animate({
        'marginLeft': margin
      }, self.opts.animate, self.opts.easing, callback);
    };
